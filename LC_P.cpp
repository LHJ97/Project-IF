...
void LC_P()
{
	...
	if (n == 1) {
		...
		cout << "|Z| : " << ModuleS(0, (2 * M_PI*fmax * L )/ (1-pow(2 * M_PI*fmax,2) * C * L)) << endl;
		cout << "Arg(Z) : " << ArgS(0, (2 * M_PI*fmax * L) / (1 - pow(2 * M_PI*fmax, 2) * C * L)) << endl;
	}
	//Second cas ou on souhaite calculer les impedances de plusieurs frequences
		...
		for (int i = 0; i < n + 1; i++) {
			...
			// Calcul du module et d'argument
			cout << "|Z|" << i + 1 << " : " << ModuleS(0, (2 * M_PI*a[i] * L) / (1 - pow(2 * M_PI*a[i], 2) * C * L)) << "\t";
			cout << "Arg(Z)" << i + 1 << " : " << ArgS(0, (2 * M_PI*a[i] * L) / (1 - pow(2 * M_PI*a[i], 2) * C * L)) << endl;
		}
	}
	...
	for (i = 0; i <= 120; i++)
	{
		...
		ampl[i] = ModuleS(0, (2 * M_PI*freq * L) / (1 - pow(2 * M_PI*freq, 2) * C * L));
		phase[i] = ArgS(0, (2 * M_PI*freq * L) / (1 - pow(2 * M_PI*freq, 2) * C * L));
		if (ampl[i] > max) {
			max = ampl[i];
		}
	}
