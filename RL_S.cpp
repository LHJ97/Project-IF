...
void RL_S()
{
	...
	if (n == 1) {
		...
		cout << "|Z| : " << ModuleS(R, 2 * M_PI*fmax * L) << endl;
		cout << "Arg(Z) : " << ArgS(R, 2 * M_PI*fmax * L) << endl;
	}
	//Second cas ou on souhaite calculer les impedances de plusieurs frequences
		...
		for (int i = 0; i < n + 1; i++) {
			...
			// Calcul du module et d'argument
			cout << "|Z|" << i + 1 << " : " << ModuleS(R, 2 * M_PI*fmax * L) << "\t";
			cout << "Arg(Z)" << i + 1 << " : " << ArgS(R, 2 * M_PI*fmax * L) << endl;
		}
	}
